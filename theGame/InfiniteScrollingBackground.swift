...
//added anchor point here
            sprites[index].anchorPoint = CGPoint(x: 0, y: 0.5)
            sprites[index].position = CGPoint(x: 0, y: sprites[index].size.height/2 + (CGFloat(index) * sprites[index].size.height))
